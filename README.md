...
| `TRIATTN_RUNTIME_FAST_RECENCY_ACCURACY_GUARD` | `true` | When stats are configured, prefer sparse TriAttention selection over pure recency for long-context correctness. If `FAST_RECENCY_ONLY=1` is explicitly set and stats are not configured, this defaults off for pure-recency diagnostics. |
